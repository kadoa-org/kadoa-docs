---
title: Subscribing to Kadoa Webhooks
...
1. Make a POST request to the `/v2/webhooks/subscribe` endpoint with the following JSON payload:
...
The `events` array should contain the list of events you want to subscribe to. You can fetch the list of available events by making a GET request to the [events endpoint](/api-reference/webhook-management/subscribe-to-webhook-events).
...
To unsubscribe from webhook events, make a DELETE request to the [unsubscribe endpoint](/api-reference/webhook-management/unsubscribe-from-webhook-events), using the ID of the subscription you want to remove.
